...
int knapsack(vector<int>&val, vector<int>&wei, int capacity, int n){
 ...
 if(wei[n-1]<=capacity){
    ...
    return max(knapsack(val,wei,capacity-wei[n-1],n-1)+val[n-1],knapsack(val,wei,capacity,n-1));
     ...
     return knapsack(val,wei,capacity,n-1);
 }
